...
describe("agent Jev parity", () => {
  ...
  it("keeps paid search tools in auto-mode", () => {
    ...
    expect(AUTO_MODE_TOOLS.has("update_pipeline")).toBe(true);
  ...
  it("exposes Field, swarm, and history list tools", () => {
    ...
    expect(source).toContain("update_pipeline:");
    ...
    expect(source).toContain("jev_triage:");
    ...
    expect(source).toContain("score_fit:");
    expect(source).toMatch(/update_entity:[\s\S]*website: z\.string\(\)/);
    expect(source).toMatch(/update_entity:[\s\S]*tags: z\.array/);
