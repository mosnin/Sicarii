...
import { authenticateOauthAccessToken, hasScope } from "@/lib/oauth-server";
...
function authError(status: number, error: string, description: string, scope?: string) {
  return Response.json(
    { error, error_description: description },
    {
      ...
      headers: {
        ...headers,
        "WWW-Authenticate": `Bearer error="${error}"${scope ? `, scope="${scope}"` : ""}`,
      },
    },
  );
}
...
export async function GET(req: Request) {
  ...
  if (!hasScope(ctx, "crm:read")) {
    return authError(403, "insufficient_scope", "The Scalar client overview requires read access", "crm:read");
  }

  const origin = scalarPublicOrigin(req);
  const expectedResource = `${origin}/api/client/v1/overview`;
  if (ctx.resource !== expectedResource) {
    return authError(403, "invalid_target", "This token was not issued for the Scalar client API");
  }
